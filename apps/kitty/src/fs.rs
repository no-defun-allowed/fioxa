...
use fioxa_rpc::{
    client::RPCClient,
    fs::{Read, Size, StatResult, stat_by_path},
...
impl File {
    pub fn open(path: &Pathname) -> Result<Self, Error> {
        ...
        let Some(server) = fs.get(path.disk) else {
            return Err(Error::InvalidDiskId);
        };
    ...
    pub fn len(&mut self) -> Result<u64, Error> {
        ...
        Ok(r.get_message().unwrap().get_size())
    }

    pub fn read(&mut self, start: u64, len: u32) -> Result<Vec<u8>, Error> {
